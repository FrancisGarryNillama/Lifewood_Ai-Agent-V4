...
function getLatestModified(items: DriveItem[]): string | null {
  ...
  for (const item of items) {
    if (item.modifiedTime) {
      ...
      if (Number.isFinite(parsed)) latestTimestamp = Math.max(latestTimestamp, parsed);
    }
    if (item.children?.length) {
      const nestedLatest = getLatestModified(item.children);
      if (nestedLatest) latestTimestamp = Math.max(latestTimestamp, Date.parse(nestedLatest));
    }
  }
...
function countItems(item: DriveItem): number {
  ...
  return item.children.reduce((total, child) => {
    return isFolder(child) ? total + 1 + countItems(child) : total + 1;
  }, 0);
}
  ...
  const greetingContent = useMemo(() => {
    if (userType === 'new') {
      return {
        ...
        description: 'Your AI workspace is ready. Connect a folder to start scanning receipts, organizing finances, and tracking activity automatically.',
      };
    }
    return {
      header:      'Welcome back!',
      description: 'Your Finance AI workspace is active. Continue reviewing scanned receipts and let AI keep your finances organized in real time.',
    };
  }, [userType]);
        ...
        <div className={styles.pageContent}>
          ...
          <section className={styles.heroBanner}>
            <div className={styles.heroBannerLeft}>
              ...
              <p className={`${styles.heroSubtitle} ${styles.greetingDescription} ${userType === 'new' ? styles.newUserDescriptionIn : styles.returningUserDescriptionIn}`}>
            ...
            {activeFolder ? (
              <>
                ...
                <article className={styles.statItemAction}>
                  ...
                  <button className={styles.statButton} onClick={() => openFolder(activeFolder.id)} type="button">
                ...
                return (
                  ...
                  >
                    ...
                    <div className={styles.folderCardBody}>
                      <h3>{folder.name}</h3>
                      <p className={styles.folderSummary}>{directItems} item{directItems === 1 ? '' : 's'}</p>
                    </div>
